...
@mcp.tool()
def list_shpigford_skills() -> dict:
    """List all available shpigford skills."""
    return {k: v["description"] for k, v in _SKILLS.items()}


@mcp.tool()
def get_shpigford_skill(skill_name: str = None) -> dict:
    """Get full guidance for a specific shpigford skill."""
    ...
    hint = get_presentation_hint('shpigford', client_id=client_id)
    if hint:
        skill_data = {**skill_data, "_presentation_hint": hint}
    return {
        "id": f"{skill_name}@shpigford",
