...
function LinkSkillSection({ onLinked }: { onLinked: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    if (!res.ok) {
      ...
      setStatusMsg({ type: "success", text: `"${skillName}" installed and activated successfully.` });
      setRepoUrl("");
      setSkillName("");
      ...
      {open && (
        <div className="space-y-3 pt-1">
          <p className="text-xs text-muted-foreground">
            Paste a public GitHub repo URL and choose a unique snake_case name. The skill will be installed
            locally and activated immediately.
          </p>
